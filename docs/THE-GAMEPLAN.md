...
| `tools/validate.py` | Schema plus policy over a JSONL corpus. `--strict`. **Delivered 2026-09-08**, with `--kernel` running the generated-artifact check, both corpora with `expect_only` and short-circuit disclosure, and a self-test | Tooling | C | Rung 2 of the ladder. Everything | schema, policy |
...
| `tools/generate_pse.py` | Writes the schema and the four policy files from `spec/layer-model.yaml` and `ontology/selectors.yaml`, and is their only writer. `--check` refuses a generated file that differs from what the model generates. Added 2026-09-08 | Tooling | C | The generated artifacts drift from their source and nothing notices | layer-model, ontology |
| `tools/build_corpus.py` | Writes `conformance/must-pass.jsonl` and `conformance/must-fail.jsonl` from its fixture tables, one synthetic case and one break per fixture, and is their only writer. `--check` refuses drift. Added 2026-09-08 | Tooling | C | A fixture can be edited by hand into passing | schema, policy |
...
**Step 7 — Write the schema, the policy pack, and the contract.** **Unblocked 2026-09-03 when Steps 5 and 6 were drafted. Delivered 2026-09-08:** the schema and the four policy files are generated by `tools/generate_pse.py`, the two corpora by `tools/build_corpus.py`, `tools/validate.py --kernel` grades them and refuses drift, and the contract was written last, UNRATIFIED, with its sections 5 and 12 as stamp targets. In this order: `schema/pse-event-0.1.schema.json` generated from the layer model, then `policy/{semantics,lineage,producer-authority,violation-codes}.yaml`, then `spec/pse-semantics-contract.md` written last, because the contract explains rules that already exist rather than inventing rules nothing enforces.
